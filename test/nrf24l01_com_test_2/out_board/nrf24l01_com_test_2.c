...
void write_cmd(volatile f10xxx_spi_t *const spi_x,
	       volatile f10xxx_gpio_t *gpio_x,
	       f10xxx_gpio_port_e nss_port,
	       u8 cmd,
	       u8 *buf,
...
void f10xxx_reset_isr(){
  ...
  */
  ...
  write_cmd(spi_1,
	    gpio_a,
	    port_4,
	    NRF24L01_WRITE_REG(NRF24L01_REG_EN_AA),
	    buf,
	    1,
	    &status);
  ...
  //write then read back and verify change

  buf[0] = (1 << 6) | (1 << 5) | (1 << 4) | (1 << 1) | 1;
  write_cmd(spi_1,
	    gpio_a,
	    port_4,
  ...
  */
  ...
  write_cmd(spi_1,
	    gpio_a,
	    port_4,
	    NRF24L01_WRITE_REG(NRF24L01_REG_RX_PW_P0),
	    buf,
	    1,
	    &status); // setup payload width = 1
  ...
  byte_to_hex(buf[1], hex);
  usart_write_string(usart_1, (u8 *) "PW_P0 : ");
  usart_write_string(usart_1, hex);
  usart_write_string(usart_1, (u8 *) "\n");
  ...
  while(1){
    ...
    buf[0] = (1 << 6); // can you ack with CE HIGH?
    write_cmd(spi_1,
	      gpio_a,
	      port_4,
